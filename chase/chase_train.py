...
from chase_dataset import *
from chase_network import *
from tqdm import tqdm

#device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
folder = r"C:\Users\vamsi\Documents\AI\data\dataset\CHASE\data\training\\"
epochs = 100
model = r2u_net()
 
model = model.cuda()
ds = chaseTrainDataset(folder) 
dl = getDataLoader(ds)
val_ds = chaseValDataset(folder)
...
def fit(model,epochs,train_dl,valid_dl,loss_fn,opt=None,metric=None):
    ...
    for epoch in range(epochs):
        ...
        metric_train_epoch.append(metric_train_epoch)
        ...
        fin = [a,b,c,d]
        file_name = "r2u_net__chase.pkl"
        open_file = open(file_name,"wb")
        pickle.dump(fin,open_file)
        open_file.close()
        torch.save({'model_state_dict':model.state_dict(),'opt_state_dict':opt.state_dict(),'epoch':epoch},"r2u_net_chase_model")

    #return lss_train_epoch,metric_train_epoch,lss_val_epoch,metric_val_epoch
fit(model,epochs,dl,val_dl,bce,opt=opt,metric=get_accuracy)
