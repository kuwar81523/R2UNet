...
class driveTrainDataset(Dataset):
    ...
    def __init__(self,folder):
        ...
        self.train_mask_list = os.listdir(folder+"\\mask_patches\\train\\")
    ...
    def __getitem__(self,idx):
        ...
        mask = read_image(self.folder+"\\mask_patches\\train\\"+mask_name)
        img = img.float()
        mask = mask.float()
        return data_aug(img,mask)

class driveValDataset(Dataset):
    
    def __init__(self,folder):
        self.val_img_list = os.listdir(folder+"\\image_patches\\val\\")
        self.val_mask_list = os.listdir(folder+"\\mask_patches\\val\\")
    ...
    def __getitem__(self,idx):
        ...
        mask = read_image(self.folder+"\\mask_patches\\val\\"+mask_name)
        img = img.float()
        mask = mask.float()
        return img,mask
