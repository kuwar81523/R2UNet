...
from drive_network import *
from tqdm import tqdm
import pickle
#device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
folder = r"C:\Users\vamsi\Documents\AI\data\dataset\DRIVE\training\\"
epochs = 100
model = r2u_net()
 
model = model.cuda()
ds = driveTrainDataset(folder) 
dl = getDataLoader(ds)
val_ds = driveValDataset(folder)
...
def fit(model,epochs,train_dl,valid_dl,loss_fn,opt=None,metric=None):
    ...
    for epoch in range(epochs):
        ...
        fin = [a,b,c,d]
        file_name = "r2u_net_drive.pkl"
        open_file = open(file_name,"wb")
        pickle.dump(fin,open_file)
        open_file.close()
        torch.save({'model_state_dict':model.state_dict(),'opt_state_dict':opt.state_dict(),'epoch':epoch},"r2u_net_drive_model")
